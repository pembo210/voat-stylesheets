...
// day of the week, check hour
$today = gmdate(l); 
$nowHour = gmdate(H);
$metaHours = array("00", "01", "02");
$daySplit = array("00", "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12");

if($today==Monday){
	if (in_array($nowHour, $daySplit)) {
		$MPmonth = "Jan"; } else { $MPmonth = "Feb";
	}
}
elseif($today==Tuesday){
	if (in_array($nowHour, $daySplit)) {
		$MPmonth = "March"; } else { $MPmonth = "April";
	}
}
elseif($today==Wednesday){
	if (in_array($nowHour, $daySplit)) {
		$MPmonth = "May"; } else { $MPmonth = "June";
	}
}
elseif($today==Thursday){
	if (in_array($nowHour, $daySplit)) {
		$MPmonth = "July"; } else { $MPmonth = "Aug";
	}
}
elseif($today==Friday){
	if (in_array($nowHour, $daySplit)) {
		$MPmonth = "Sept"; } else { $MPmonth = "Oct";
	}
}
elseif($today==Saturday){
	if (in_array($nowHour, $daySplit)) {
		$MPmonth = "Nov"; } else { $MPmonth = "Dec";
	}
}
elseif($today==Sunday){
	if (in_array($nowHour, $metaHours)) {
		$MPmonth = "Dec"; } else { $MPmonth = "Meta";
	}
}

//find MPdate off hour
$nHour = gmdate(h);
if($nHour=="00") {$MPdate = "1st";}
elseif($nHour=="01") {$MPdate = "3rd";}
elseif($nHour=="02") {$MPdate = "5th";}
elseif($nHour=="03") {$MPdate = "8th";}
elseif($nHour=="04") {$MPdate = "11th";}
elseif($nHour=="05") {$MPdate = "13th";}
elseif($nHour=="06") {$MPdate = "16th";}
elseif($nHour=="07") {$MPdate = "18th";}
elseif($nHour=="08") {$MPdate = "20th";}
elseif($nHour=="09") {$MPdate = "22nd";}
elseif($nHour=="10") {$MPdate = "24th";}
elseif($nHour=="11") {$MPdate = "26th";}
elseif($nHour=="12") {$MPdate = "28th";}
